...
__global__ void mult(unsigned *matrix,unsigned *result ,unsigned size){
    
    unsigned id =blockDim.x*blockIdx.x+threadIdx.x;
    unsigned i=id/size;
    unsigned j=id%size;
    for(unsigned k=0;k<size;k++){
    result[i*size+j]+=matrix[i*size+k]*matrix[k*size+j];
 }
} // end mult
